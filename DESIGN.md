...
- **Masthead**: sticky, logo left (real transparent PNG, `origin-talent-black-transparent.png`, rendered at 48px tall — up from the original 34px so it reads clearly against the paper background), contact right (collapses to a hamburger + drawer rack under 900px). Masthead height grew to ~76px with the larger logo, so the sticky `.rack` sidebar's `top`/`height` offset was bumped from 65px to 79px, and `.forms`'s `scroll-margin-top` from 90px to 104px, to keep both in sync with the taller bar.
