...
- Logo supplied in light-on-dark and dark-on-light variants, with a secondary lockup adding "EXCEPTIONAL PEOPLE FOR EXCEPTIONAL HOMES" / "WWW.ORIGINTALENT.CO.ZA" / "LOCATED IN JOHANNESBURG" in small caps beneath. Real files (PNG/SVG/JPG/PDF, black-on-light and white-on-dark) live at `C:\Users\user\OneDrive\Origin Talent\` and are mirrored into `assets/brand/`.
- **Current logo files in use are the genuinely transparent PNGs**: `assets/brand/origin-talent-black-transparent.png` (masthead + footer, all pages) and `assets/brand/origin-talent-lockup-black-transparent.png` (full lockup with tagline, used on the Contact Us page). These supersede the earlier `origin-talent-black.svg`/`.png` and `origin-talent-white.svg`/`.png`, which had a baked-in white background rather than real alpha transparency — verified via `GetPixel().A` before swapping. The old files are still present in `assets/brand/` but should be treated as deprecated, not deleted (in case a dark-background placement is ever needed and a genuinely transparent white version is sourced).
...
- **Pinned palette, confirmed directly by the user — grayscale only, no color accent**: `#f0f0f0` (paper/background), `#474747` and `#444444` (ink, two closely-related dark grays), `#ffffff` (white). No navy, gold, or any hue appears in the real brand — this explicitly overrides any earlier color direction invented before this was confirmed.
